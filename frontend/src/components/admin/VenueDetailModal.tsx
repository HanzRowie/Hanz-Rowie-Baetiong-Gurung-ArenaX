/**
 * VenueDetailModal Component
 * 
 * Modal dialog for viewing detailed venue information and performing approval actions.
 * Requirements: 9.5, 9.6, 9.8, 10.1-10.6, 13.6, 14.2, 16.4, 17.2, 20.4, 20.6, 26.6
 */

import React, { useEffect, useState } from 'react';
import type { AdminVenue } from '../../types/verification.types';
...
interface VenueDetailModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onApprove: (venueId: string, notes?: string) => void;
  onReject: (venueId: string, reason: string) => void;
  onConditionalApprove: (venueId: string, requestedDocuments: string[]) => void;
  isLoading?: boolean;
}
...
export const VenueDetailModal: React.FC<VenueDetailModalProps> = ({
  ...
  }, [isOpen, venue, showApprovalDialog, showRejectionDialog, showConditionalDialog, onClose]);
  ...
  const isConditional = venue.approval_status === 'CONDITIONAL_APPROVAL';

  const handleApprove = () => {
    onApprove(venue.id, approvalNotes || undefined);
    setShowApprovalDialog(false);
    setApprovalNotes('');
  };
  ...
  const handleConditionalApprove = () => {
    if (requestedDocs.length > 0) {
      onConditionalApprove(venue.id, requestedDocs);
      setShowConditionalDialog(false);
      setRequestedDocs([]);
    }
  };
  ...
  return (
    <>
        ...
        <div className="flex min-h-full items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="relative bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full sm:max-w-5xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs sm:text-sm text-gray-500">
                    Complete venue information and verification
                  </p>
                </div>
                <button
                  onClick={onClose}
            ...
            <div className="overflow-y-auto max-h-[calc(95vh-140px)] sm:max-h-[calc(90vh-180px)] px-4 sm:px-6 py-3 sm:py-4">
              {/* Basic Information (Requirement 9.5) */}
              <section className="mb-4 sm:mb-6">
                <h4 className="text-sm sm:text-md font-semibold text-gray-900 mb-2 sm:mb-3">Venue Information</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  {venue.capacity && (
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-gray-500">Capacity</label>
                      <p className="mt-1 text-xs sm:text-sm text-gray-900">{venue.capacity} people</p>
                    </div>
                  )}
                  <div>
              ...
              {/* Owner Details (Requirement 9.5) */}
              <section className="mb-4 sm:mb-6">
                <h4 className="text-sm sm:text-md font-semibold text-gray-900 mb-2 sm:mb-3">Owner Details</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-gray-500">Name</label>
                    <p className="mt-1 text-xs sm:text-sm text-gray-900 break-words">
              ...
              {/* Operating Hours (Requirement 9.5) */}
              <section className="mb-4 sm:mb-6">
              ...
              {/* Pricing Information (Requirement 9.5) */}
              <section className="mb-4 sm:mb-6">
              ...
              {/* Approval Status and History (Requirement 9.5, 17.2) */}
              <section className="mb-4 sm:mb-6">
                ...
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      ...
                      >
                        {venue.approval_status.replace('_', ' ')}
                      </span>
                    </div>
                    {venue.approval_date && (
                      <div>
                        ...
                        <p className="mt-1 text-xs sm:text-sm text-gray-900">{formatDate(venue.approval_date)}</p>
                      </div>
                    )}
                    {venue.approved_by_name && (
                      <div>
                        <label className="block text-xs sm:text-sm font-medium text-gray-500">
                          {isApproved ? 'Approved By' : isRejected ? 'Rejected By' : 'Processed By'}
                        </label>
                        <p className="mt-1 text-xs sm:text-sm text-gray-900 break-words">{venue.approved_by_name}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {isRejected && venue.rejection_reason && (
                    <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-red-50 border border-red-200 rounded-md">
                      ...
                      <p className="text-xs sm:text-sm text-red-700 break-words">{venue.rejection_reason}</p>
                    </div>
                  )}

                  {/* Approval Notes */}
                  {venue.approval_notes && (
                    <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-green-50 border border-green-200 rounded-md">
                      ...
                      <p className="text-xs sm:text-sm text-green-700 break-words">{venue.approval_notes}</p>
                    </div>
                  )}

                  {/* Requested Documents for Conditional Approval */}
                  {isConditional && venue.requested_documents && venue.requested_documents.length > 0 && (
                    <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-blue-50 border border-blue-200 rounded-md">
                      ...
                      <ul className="list-disc list-inside text-xs sm:text-sm text-blue-700">
                        {venue.requested_documents.map((doc, index) => (
                          <li key={index}>{doc}</li>
                        ))}
                      </ul>
                    </div>
              ...
              {/* Validation Results (Requirement 13.6) */}
              {venue.validation_results && venue.validation_results.length > 0 && (
                <section className="mb-4 sm:mb-6">
                  <h4 className="text-sm sm:text-md font-semibold text-gray-900 mb-2 sm:mb-3">Validation Results</h4>
                  <div className="space-y-2">
                    {venue.validation_results.map((result, index) => (
                      <div
                        key={index}
              ...
              <section className="mb-4 sm:mb-6">
                ...
                {venue.verification_documents && venue.verification_documents.length > 0 ? (
                  <div className="space-y-4">
                    {venue.verification_documents.map((doc, index) => (
                      <DocumentViewer
                        key={index}
              ...
              {venue.audit_logs && venue.audit_logs.length > 0 && (
                <section className="mb-4 sm:mb-6">
            ...
            {/* Footer with action buttons (Requirement 9.6, 9.8, 14.2, 20.6) */}
            <div className="bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 sticky bottom-0">
              ...
              <button
                ...
                aria-label={isApproved ? 'Venue already approved' : 'Approve venue'}
              >
      ...
      {/* Approval Confirmation Dialog (Requirement 9.6, 20.6) */}
      {showApprovalDialog && (
        <div className="fixed inset-0 z-[60] overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Approve Venue</h3>
      ...
      {/* Rejection Dialog (Requirement 9.6, 20.6) */}
      {showRejectionDialog && (
        <div className="fixed inset-0 z-[60] overflow-y-auto">
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75" onClick={() => setShowRejectionDialog(false)} />
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Reject Venue</h3>
              <p className="text-sm text-gray-600 mb-4">
        ...
        <div className="fixed inset-0 z-[60] overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Select the documents you need from the venue owner before final approval.
              </p>
              
              <div className="space-y-2 mb-6">
                {['Business License', 'Facility Photos', 'Insurance Certificate', 'Operating Permit', 'Other Documentation'].map((doc) => (
