/**
 * VenueFilterBar Component
 * 
 * Provides filtering controls for the admin venue management interface.
 * Includes sport_type filter, status filter, date range picker, and search input.
 * 
 * Requirements:
 * - 9.2: Display filter bar with sport_type, status, owner, date_range filters
 * - 9.4: Search input with operator support
 * - 16.7: Implement filter persistence in local storage
 * - 18.2, 18.3: Search with advanced operators
 */

import React, { useState, useEffect, useCallback } from 'react';
import { ChevronDown, ChevronUp } from 'lucide-react';
import type { VenueFilters, SportType, ApprovalStatus } from '@/types/verification.types';
 ...
 */
const FILTER_STORAGE_KEY = 'admin_venue_filters';

/**
 * VenueFilterBar Component
 * 
 * Provides comprehensive filtering controls for venue management.
 ...
 */
export const VenueFilterBar: React.FC<VenueFilterBarProps> = ({ filters, onFiltersChange }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-4">
          <div className="relative">
            ...
            <input
              ...
              aria-label="Search venues by name, owner, or address"
            />
          </div>
        </div>
        ...
        <div className="md:col-span-3">
          <input
            ...
            className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
            aria-label="Filter by venue submission date"
          />
        </div>
